...
#if defined(NDEBUG)
# error "Uemcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xc12eeebe1b8cc0715304752c4a2d3a55d804af7f57de1d9fceaf58ca6682d8f3");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Uemcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Uemcoin Signed Message:\n";
...
{
    // Uemcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 100000); // Uemcoin: 100k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Uemcoin: 1 days
static const int64 nTargetSpacing = 3 * 60; // Uemcoin: 3 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Uemcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Uemcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x8090f6c1a54b8a8e24c0f4038732e2a7acd26e4b05412f905a3d48a1dcbfb785");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Doug Jones defeats Roy Moore in Alabama election";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 33 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0493f4a634e3b6461297eee1c9f696dd22770323cb4dad0bb911a270ee6dc0f74517e9deadf07d449a3fa04517ba387c3c5a8f31d641f7f7e37a317e1097ce94d6") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1522253184;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2089823199;

        if (fTestNet)
        {
            block.nTime    = 1522253153;
            block.nNonce   = 386087207;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xb12ab4ccff2619008a00211605fdbc178acca1d7c7a379fc9a0e75fed99b4256"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf5, 0xcf, 0xb3, 0xd7 }; // Uemcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("UemcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("UemcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static UemcoinMiner(CWallet *pwallet)
{
    printf("UemcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("uemcoin-miner");
    ...
    try { loop {
        ...
        printf("Running UemcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("UemcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&UemcoinMiner, pwallet));
}
